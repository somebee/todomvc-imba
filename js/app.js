...
/***/ function(module, exports, __webpack_require__) {
	...
	_T.defineTag('todo', 'li', function(tag){
		...
		tag.prototype.render = function (){
			...
			return this.flag('completed',(this.data().completed())).setChildren([
				(__.A = __.A || _T.DIV(this).flag('view')).setContent([
					(__.AA = __.AA || _T.LABEL(this).setHandler('dblclick','edit',this)).setContent(this.data().title(),3).end(),
					(__.AB = __.AB || _T.INPUT(self).flag('toggle').setType('checkbox').setHandler('tap','toggle',self)).setChecked((self.data().completed())).end(),
					(__.AC = __.AC || _T.BUTTON(self).flag('destroy').setHandler('tap','drop',self)).end()
				],2).end(),
				(self._edit || _T.INPUT(self).ref_('edit',self).setType('text').setHandler('blur','save',self)).end()
			],2).synced();
		};
		
		tag.prototype.toggle = function (){
			return this.up(q$('._app',this)).toggle(this.data());
		};
		...
		tag.prototype.edit = function (){
			this.flag('editing');
			this._edit.setValue(this.data().title());
		...
		tag.prototype.save = function (){
			if (this.hasFlag('editing')) {
				this.unflag('editing');
				return this.up(q$('._app',this)).rename(this.data(),this._edit.value());
			};
		};
		
		tag.prototype.drop = function (){
			return this.up(q$('._app',this)).drop(this.data());
		};
	...
	_T.defineTag('app', function(tag){
		...
		tag.prototype.render = function (){
			...
			return this.setChildren([
				(__.A = __.A || _T.HEADER(this).flag('header')).setContent([
					(__.AA = __.AA || _T.H1(this).setText("todos")).end(),
					(__.AB = __.AB || _T.FORM(self)).setContent((self._adder || _T.INPUT(self).ref_('adder',self).flag('new-todo').setType('text').setPlaceholder('What needs to be done?')).end(),2).end()
				],2).end(),
				
				(Imba.len(self.data()) > 0) ? (Imba.static([
					(__.B = __.B || _T.SECTION(self).flag('main')).setContent([
						(__.BA = __.BA || _T.INPUT(self).flag('toggle-all').setType('checkbox').setHandler('change','toggleAll',self)).setChecked((Imba.len(active) == 0)).end(),
						(__.BB = __.BB || _T.UL(self).flag('todo-list')).setContent(
							(function() {
								var $BBA = (__.BBA = __.BBA || {});
								for (var i = 0, ary = Imba.iterable(items), len = ary.length, todo, res = []; i < len; i++) {
									todo = ary[i];
									res.push(($BBA[todo.id()] = $BBA[todo.id()] || _T.TODO(self)).setData(todo).end());
								};
								return res;
							})()
						,3).end()
					...
					(__.C = __.C || _T.FOOTER(self).flag('footer')).setContent([
						(__.CA = __.CA || _T.SPAN(self).flag('todo-count')).setContent([
							(__.CAA = __.CAA || _T.STRONG(self)).setContent(("" + (Imba.len(active)) + " "),3).end(),
							(Imba.len(active) == 1) ? ('item left') : ('items left')
						],1).end(),
						(__.CB = __.CB || _T.UL(self).flag('filters')).setContent([
							(__.CBA = __.CBA || _T.LI(self)).setContent((__.CBAA = __.CBAA || _T.A(self).setHref('#/').setText('All')).flag('selected',(items == self.data())).end(),2).end(),
						...
						(Imba.len(done) > 0) ? (
							(__.CC = __.CC || _T.BUTTON(self).flag('clear-completed').setHandler('tap','archive',self).setText('Clear completed')).end()
						) : void(0)
					],1).end()
				],2)) : void(0)
			],1).synced();
		...
		tag.prototype.remaining = function (){
			return this.data().filter(function(todo) { return !todo.completed(); });
		};
		...
		tag.prototype.add = function (title){
			if (title.trim()) {
				this.data().push(new Todo(title.trim()));
				return this.persist();
			};
		};
		...
		tag.prototype.toggleAll = function (e){
			for (var i = 0, ary = Imba.iterable(this.data()), len = ary.length; i < len; i++) {
		...
		tag.prototype.drop = function (todo){
			...
			this.setData(this.data().filter(function(t) { return t != todo; }));
			return this.persist();
		};
		
		// remove all completed todos from collection
		tag.prototype.archive = function (){
			this.setData(this.remaining());
			return this.persist();
		};
		...
		tag.prototype.load = function (){
			...
			this.setData(items.map(function(item) { return new Todo(item.title,item.completed); }));
			return this;
		};
		...
		tag.prototype.persist = function (){
			...
			if (json != this._json) {
				window.localStorage.setItem('todos-imba',this._json = json);
			};
	...
	Imba.mount(_T.APP(this).setId('app').end(),document.querySelector('.todoapp'));
