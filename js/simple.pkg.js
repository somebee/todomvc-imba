...
(function(){
	...
	Imba.defineSingletonTag = function (id,supr,body){
		...
		// should drop this in production / optimized mode, but for debug
		// we create a constructor with a recognizeable name
		var fun = new Function(("return function " + (id.substr(1).replace(/[\s\-\:]/g,'_')) + "(dom)\{ this.setDom(dom); \}"));
		var singleton = fun();
	...
	Imba.extendTag('htmlelement', function(tag){
		...
		tag.prototype.__title = {dom: true,name: 'title'};
		...
		tag.prototype.__role = {dom: true,name: 'role'};
					...
					(t0 = self.$b || (self.$b = t$('section'))).flag('main').setContent(Imba.static([
						...
						(t1 = t0.$$b || (t0.$$b = t$('ul'))).flag('todo-list').setContent(Imba.static([(function(t1) {
							for (var i=0, ary=iter$(items), len=ary.length, todo, res=[]; i < len; i++) {
								todo = ary[i];
								res.push((t2 = t1['_' + todo.id()] || (t1['_' + todo.id()] = t$('li'))).flag('completed',todo.completed()).flag('editing',todo.editing()).setObject(todo).setContent(Imba.static([
									(t3 = t2.$$a || (t2.$$a = t$('div'))).flag('view').setContent(Imba.static([
										(t3.$$a = t3.$$a || t$('label')).setHandler('dblclick',['edit',todo]).setContent(Imba.static([todo.title()],1)).end(),
										(t3.$$b = t3.$$b || t$('input')).flag('toggle').setType('checkbox').setHandler('tap',['toggle',todo]).setChecked(todo.completed()).end(),
										(t3.$$c = t3.$$c || t$('button')).flag('destroy').setHandler('tap',['drop',todo]).end()
									],1)).end(),
									(todo.editing()) && (
										(t2.$$b = t2.$$b || t$('edit_todo')).flag('edit').setType('text').setObject(todo).end()
									)
								],1)).end());
							};
							return res;
